...
class Car extends BaseModel
{
    ...
    public function canDelete() {
        if($this->created_by == Auth::guard('admin')->user()->id && $this->bill()->count() == 0 || Auth::guard('admin')->user()->type == 1 && $this->bill()->count() == 0 ) {
            return true;
        } else {
            return false;
        }
    ...
    public function canEdit() {
        if($this->created_by == Auth::guard('admin')->user()->id || Auth::guard('admin')->user()->type == 1 ) {
            return true;
        } else {
            return false;
        }
    ...
    public static function searchByFilter($request) {
        ...
        if(empty($request->license_plate) && empty($request->customer_mobile)) {
            if(Auth::guard('admin')->user()->type == User::G7 || Auth::guard('admin')->user()->type == User::NHAN_VIEN_G7) {
                $result = $result->where('g7_id', Auth::guard('admin')->user()->g7_id);
            }
            if(Auth::guard('admin')->user()->type == User::NHOM_G7) {
                $g7_ids = User::find(Auth::guard('admin')->user()->id)->g7s->g7_id;
