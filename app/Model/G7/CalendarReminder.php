...
class CalendarReminder extends BaseModel
{
    ...
    public function canDelete() {
        return Auth::guard('admin')->user()->id == $this->created_by || Auth::guard('admin')->user()->type == 3 && Auth::guard('admin')->user()->g7_id == $this->g7_id;
    }
    ...
    public static function searchByFilter($request) {
        $result = self::where('g7_id', Auth::guard('admin')->user()->g7_id)
            ->with([
                'customer',
                'car' => function($q) {
                    $q->with([
