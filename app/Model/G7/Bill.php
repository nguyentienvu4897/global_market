...
class Bill extends BaseModel
{
    ...
    {
        return $this->g7_id == Auth::guard('admin')->user()->g7_id && ($this->status == self::DA_DUYET || $this->status == self::DA_CHOT);
    }
    ...
    public function canDelete() {
        return $this->status == self::DANG_TAO && Auth::guard('admin')->user()->id == $this->created_by;
    }
    ...
    public function canView() {
        return Auth::guard('admin')->user()->id == $this->created_by || Auth::guard('admin')->user()->is_super_admin;
    }

	public function canExport() {
        return $this->status == self::DA_DUYET && in_array(Auth::guard('admin')->user()->type, [User::G7]) && Auth::guard('admin')->user()->g7_id == $this->g7_id;
    }

	public function canReturn() {
        return $this->status == self::DA_CHOT && in_array(Auth::guard('admin')->user()->type, [User::G7]) && Auth::guard('admin')->user()->g7_id == $this->g7_id;
    }

    public static function searchByFilter($request) {
        $result = self::query();

        if (Auth::guard('admin')->user()->type == User::SUPER_ADMIN || Auth::guard('admin')->user()->type == User::QUAN_TRI_VIEN) {

        } else if (Auth::guard('admin')->user()->type == User::NHOM_G7) {

        } else $result = $result->where('g7_id', Auth::guard('admin')->user()->g7_id);

		if ($request->type == 'export') {
			$result = $result->where('status', self::DA_DUYET);
		} else if ($request->type == 'return') {
			$result = $result->where('status', self::DA_CHOT)->where('g7_id', Auth::guard('admin')->user()->g7_id);
		}
    ...
    public static function getReportByDates($from_date, $to_date)
    {
        $rsl = self::query()
            ...
            ->where('g7_id', Auth::guard('admin')->user()->g7_id)
            ->first();
	...
	public static function promoReportQuery($request) {
		...
		if (Auth::guard('admin')->user()->type == User::G7) {
			$result = $result->where('b.g7_id', Auth::guard('admin')->user()->g7_id);
		}
