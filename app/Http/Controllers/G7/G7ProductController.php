...
class G7ProductController extends Controller
{
	...
	public function store(Request $request)
	{
			...
			$rule = array_merge($rule, [
				'name' => [
					...
					Rule::unique('g7_products')->where(function ($query) use ($request) {
						return $query->where('g7_id', Auth::guard('admin')->user()->g7_id);
					}),
				],
				'code' => [
					'required',
		...
		try {
			...
			if ($request->root_product_id) {
				$exist = ThisModel::where('root_product_id', $request->root_product_id)->where('g7_id', Auth::guard('admin')->user()->g7_id)->first();
			...
			$object->status = 1;
			$object->g7_id = Auth::guard('admin')->user()->g7_id;
			$object->save();
    ...
    public function updatePrice(Request $request)
    {
        ...
        if ($product_id && $price) {
            G7ProductPrice::updateOrCreate(
                [
                    'g7_id' => Auth::guard('admin')->user()->g7_id,
